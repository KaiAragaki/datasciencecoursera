rankhospital <- function(state, outcome, num = "best"){
  ...
  if(sum(grepl(paste0("^",state,"$"), data[,2]))==0){
    ...
    stateCondition <- stateCondition[order(stateCondition[,3], stateCondition[,1]),]
    ranks <- 1:nrow(stateCondition)
    stateCondition <- cbind(stateCondition, ranks)
    if (num == "best") {
      hospital <- stateCondition[1,1]
    }
    else if (num == "worst") {
      hospital <- stateCondition[nrow(stateCondition),1]
    }
    else {
      hospital <- stateCondition[num,1]
    }
  }
  hospital
}
